...
const mainLoop = async (providers: ShadowKeyProviders, walletCtx: api.WalletContext, rli: Interface): Promise<void> => {
  ...
  while (true) {
    const dustLabel = await getDustLabel(walletCtx.wallet);
    const choice = await rli.question(shadowkeyMenu(dustLabel));
    switch (choice.trim()) {
      case '1':
        try {
...
export const run = async (config: Config, _logger: Logger, dockerEnv?: DockerComposeEnvironment): Promise<void> => {
  ...
  try {
    ...
    if (dockerEnv !== undefined) {
      ...
      if (config instanceof UndeployedConfig) {
        config.indexer = mapContainerPort(env, config.indexer, 'shadowkey-indexer');
        config.indexerWS = mapContainerPort(env, config.indexerWS, 'shadowkey-indexer');
        config.node = mapContainerPort(env, config.node, 'shadowkey-node');
        config.proofServer = mapContainerPort(env, config.proofServer, 'shadowkey-proof-server');
      }
    }
