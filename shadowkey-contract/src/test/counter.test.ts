import { shadowkeySimulator, logger } from "./simulators/simulator.js";
...
function createSimulator() {
  const simulator = shadowkeySimulator.deployContract(key1);
...
describe("shadowkey smart contract", () => {
  it("Display intial values", () => {
    ...
    expect(initialPrivateState).toEqual({ privateshadowkey: 0 });
  ...
  it("increments the shadowkey correctly", () => {
    const simulator = createSimulator();
    ...
    expect(nextPrivateState).toEqual({ privateshadowkey: 0 });
    const circuitContext = simulator.as("p1").getCircuitContext();
