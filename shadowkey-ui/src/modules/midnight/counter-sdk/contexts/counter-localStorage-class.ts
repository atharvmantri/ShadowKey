...
export class LocalStorage implements LocalStorageProps {
  ...
  addContract(contract: string): void {
    ...
    const item = window.localStorage.getItem('shadowkey_contracts');    
    const contracts: string[] = item ? JSON.parse(item) : [];
    const updatedContracts = Array.from(new Set([...contracts, contract]));
    window.localStorage.setItem('shadowkey_contracts', JSON.stringify(updatedContracts));
  }
  ...
  //   return window.localStorage.getItem('shadowkey_contractPrivateId' + contract);
  // }
  ...
  //   window.localStorage.setItem('shadowkey_contractPrivateId' + contract, contractPrivateId);
  // }
}
