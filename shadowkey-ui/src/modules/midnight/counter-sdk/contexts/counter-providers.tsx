...
import { shadowkeyProviders } from "../api/common-types";
...
import { shadowkeyPrivateState } from "@eddalabs/shadowkey-contract";
import {
  fromHex,
  toHex,
...
export interface ProvidersState {
  privateStateProvider: PrivateStateProvider<typeof shadowkeyPrivateStateId>;
  zkConfigProvider?: ZKConfigProvider<shadowkeyCircuits>;
...
export const Provider = ({ children, logger }: ProviderProps) => {
  ...
  const privateStateProvider: PrivateStateProvider<
    typeof shadowkeyPrivateStateId
  > = useMemo(
    () =>
      // new WrappedPrivateStateProvider(
      //   levelPrivateStateProvider({
      //     privateStateStoreName: "shadowkey-private-state",
      //   }),
      //   logger
      // ),
      inMemoryPrivateStateProvider<string, shadowkeyPrivateState>(),
    [logger, status]
  );
  ...
  const zkConfigProvider = useMemo(() => {
    ...
    return new CachedFetchZkConfigProvider<shadowkeyCircuits>(
      `${window.location.origin}/midnight/shadowkey`,
      fetch.bind(window),
      () => {}
    );
  }, [status]);
